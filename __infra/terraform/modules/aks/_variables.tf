...
variable "vm_size" {
  description = "The size of the Virtual Machine, such as Standard_D2ds_v42."
  type        = string
...
variable "log_analytics_workspace_id" {
  description = "(Optional) log analytics workspace to connect to AKS. If enable_log_analytics_workspace is true, and this is omitted, a new workspace will be created"
  type        = string
  default     = null
}
...
variable "workload_identity_enabled" {
  type        = bool
  description = "(Optional) Enable workload identity for the cluster, istalled as add-on"
  default     = false
}

variable "oidc_issuer_enabled" {
  type        = bool
  description = "(Optional) Enable oidc issuer for the cluster"
  default     = false
}
