...
func Test_NewFileManager(t *testing.T) {
	t.Run("file system", func(t *testing.T) {
		// Create File Manager
		tmpPath := filepath.Join(os.TempDir(), fmt.Sprintf("openghg-test-file-manager-%d", rand.Int31()))
		...
		downloadFile, err := os.CreateTemp("", "openghg-test-file-manager-*")
		require.NoError(t, err)
		err = fm.Download(context.TODO(), downloadFile)
		require.ErrorIs(t, err, ErrNotFound)

		// Create a file and add content to it
		uploadFile, err := os.CreateTemp("", "openghg-test-file-manager-*")
		require.NoError(t, err)
